                    ...
                    <div class="card">
                        <div class="card-header border-0">
                            ...
                            <div class="card-tools">
                                <button type="button" class="btn btn-primary btn-sm shadow-btn" style="border-radius: 25px 25px 25px 25px;" onclick="fcShowCallResultModalByID('new','');"><span class="fas fa-plus"></span> เพิ่มข้อมูล</button>
                            </div>
                        </div>
                        <div class="card-body table-responsive" style="padding: 20px !important;" id="CallResultTableBody" >
...
<script>
    ...
    *********************************************************/  
    function fcShowCallResultModalByID(mode = 'readonly', id = null){       
        $('#NewFormModal').html();
        $.ajax({
            url: '<?=base_url('CallResult/FcFetchCallResultModal');?>',
            type: 'POST',
            data: { mode: mode, id: id },             
            success: function(response) {                
                $('#NewFormModal').html(response);
                $('#CallResultModal').modal({ backdrop: 'static', keyboard: false }).modal('show'); // Prevent closing when clicking outside
            },
    ...
    *********************************************************/  
    function FcEditCallResultModalByID(mode = 'readonly', id = null){       
        $('#NewFormModal').html();
        $.ajax({
            url: '<?=base_url('CallResult/FcFetchCallResultModal');?>',
            type: 'POST',
    ...
    *********************************************************/ 
    function fcReloadCallResultTable() {
        $.get('CallResult/FcCallResultTables', function(response) {
            $('#CallResultTableBody').html(response);
        }).fail(function() {
     ...
     *********************************************************/  
    function FcDelCallResultByID(id = null) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: '<?=base_url('CallResult/FcDelCallResult');?>',
                    type: 'POST',
                    data: { id: id },
